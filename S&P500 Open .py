...
# 1. 주식 데이터 다운로드 (코카콜라)
stock = yf.download("^GSPC ", period="10y")

# MultiIndex가 적용된 경우, 컬럼을 단순화
if isinstance(stock.columns, pd.MultiIndex):
    stock.columns = stock.columns.get_level_values(0)  # 첫 번째 레벨의 컬럼 이름만 사용

# 'Open' 열이 없으면 'Adj Close' 사용
if "Open" not in stock.columns:
    print(" 'Open' 열이 없습니다. 'Adj Low'를 대신 사용합니다.")
    stock["Open"] = stock["Adj Low"]
...
Open_prices = stock["Open"].dropna().values
stock = stock.dropna(subset=["Open"])  # NaN이 있는 행 제거

# 이동평균 계산
sma20 = moving_average(Open_prices, 20)
sma200 = moving_average(Open_prices, 200)
...
# NaN이 없는 데이터만 필터링
valid_data = stock.dropna(subset=["Open", "SMA20", "SMA200"])

# 3. 상관계수 계산
corr_matrix = np.corrcoef(valid_data[["Open", "SMA20", "SMA200"]].dropna().T)
corr_df = pd.DataFrame(corr_matrix, index=["Open", "SMA20", "SMA200"], columns=["Open", "SMA20", "SMA200"])

print(f"20일 이동평균선과 종가의 상관계수: {corr_df.loc['Open', 'SMA20']:.4f}")
print(f"200일 이동평균선과 종가의 상관계수: {corr_df.loc['Open', 'SMA200']:.4f}")
...
plt.plot(stock.index, stock["Open"], label="Open Price", color="black", alpha=0.6)
...
plt.title("S&P500 시가 이동 평균과 회귀선")
plt.legend()
plt.show()
...
plt.title("Correlation Heatmap: 시가 MV")
plt.show()
