...
if "High" not in stock.columns:
    print(" 'High' 열이 없습니다. 'Adj High'를 대신 사용합니다.")
    stock["High"] = stock["Adj High"]
...
High_prices = stock["High"].dropna().values
stock = stock.dropna(subset=["High"])  # NaN이 있는 행 제거

# 이동평균 계산
sma20 = moving_average(High_prices, 20)
sma200 = moving_average(High_prices, 200)
...
# NaN이 없는 데이터만 필터링
valid_data = stock.dropna(subset=["High", "SMA20", "SMA200"])

# 3. 상관계수 계산
corr_matrix = np.corrcoef(valid_data[["High", "SMA20", "SMA200"]].dropna().T)
corr_df = pd.DataFrame(corr_matrix, index=["High", "SMA20", "SMA200"], columns=["High", "SMA20", "SMA200"])

print(f"20일 이동평균선과 종가의 상관계수: {corr_df.loc['High', 'SMA20']:.4f}")
print(f"200일 이동평균선과 종가의 상관계수: {corr_df.loc['High', 'SMA200']:.4f}")
...
plt.plot(stock.index, stock["High"], label="High Price", color="black", alpha=0.6)
...
plt.title("Correlation Heatmap: 상한가 MV")
plt.show()
