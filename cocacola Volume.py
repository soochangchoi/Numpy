...
# 'Volume' 열이 없으면 'Adj Close' 사용
if "Volume" not in stock.columns:
    print(" 'Volume' 열이 없습니다. 'Adj Low'를 대신 사용합니다.")
    stock["Volume"] = stock["Adj Low"]
...
Volume_prices = stock["Volume"].dropna().values
stock = stock.dropna(subset=["Volume"])  # NaN이 있는 행 제거

# 이동평균 계산
sma20 = moving_average(Volume_prices, 20)
sma200 = moving_average(Volume_prices, 200)
...
# NaN이 없는 데이터만 필터링
valid_data = stock.dropna(subset=["Volume", "SMA20", "SMA200"])

# 3. 상관계수 계산
corr_matrix = np.corrcoef(valid_data[["Volume", "SMA20", "SMA200"]].dropna().T)
corr_df = pd.DataFrame(corr_matrix, index=["Volume", "SMA20", "SMA200"], columns=["Volume", "SMA20", "SMA200"])

print(f"20일 이동평균선과 종가의 상관계수: {corr_df.loc['Volume', 'SMA20']:.4f}")
print(f"200일 이동평균선과 종가의 상관계수: {corr_df.loc['Volume', 'SMA200']:.4f}")
...
plt.plot(stock.index, stock["Volume"], label="Volume amount", color="black", alpha=0.6)
...
plt.title("Correlation Heatmap: 거래량 MV")
plt.show()
